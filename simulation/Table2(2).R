...
## Screening22
##
#       function simulating cases in Table 2, part 2
#
# Input: - sim : the number of replications
#        - n: sample size 
#        - p: dimension of covariates
#        - CensRateset: setup for censoring rate
#        - ErrorNorm: Error follows from standard normal or standard Gumbel distributions
#
...
Screening22 <- function(sim = 200,n = 200,p = 5000,CensRateset = 20,ErrorNorm = 1){
  set.seed(999)
  ...
  while(m<= sim){
    try({
      ...
      if(ErrorNorm == 1){
        TT[kappa1] <- sapply(kappa1,function(i) exp((Z[i,1])^3+1.5*Z[i,2]+2*sin(Z[i,3])+exp(Z[i,4])+(2+Z[i,5])^2+rnorm(1)))
        TT[kappa2] <- sapply(kappa2,function(i) exp(0.8*Z[i,1]+0.8*Z[i,2]+0.8*Z[i,3]+0.8*Z[i,4]+0.8*Z[i,5]+rnorm(1)))
      }else{
        TT[kappa1] <- sapply(kappa1,function(i) exp((Z[i,1])^3+1.5*Z[i,2]+2*sin(Z[i,3])+exp(Z[i,4])+(2+Z[i,5])^2+rgumbel(1)))
        TT[kappa2] <- sapply(kappa2,function(i) exp(0.8*Z[i,1]+0.8*Z[i,2]+0.8*Z[i,3]+0.8*Z[i,4]+0.8*Z[i,5]+rgumbel(1)))
      }
      
      
      rec_inf = which(TT == "Inf")
      if(length(rec_inf) == 0){
        TT = TT
        Z = Z
        lenT = n
      ...
      if(CensRateset == 20){
        C = runif(lenT,0,quantile(TT,0.83))
      }else if(CensRateset == 40){
        C = runif(lenT,0,quantile(TT,0.67))
      }else if(CensRateset == 60){
        C = runif(lenT,0,quantile(TT,0.55))
      }
      ...
      filenames = paste("./simulation/Table2(2)/",'p',p,'+',
                        'C',CensRateset,'+','Norm',ErrorNorm,"+",'CRSIS','.csv',sep = '')
      ...
      CondUtility <- function(i){
        ...
        ssLogL <- function(sbeta) -sLogLiklihood(sdata,sbeta)
        results <- optim(rep(0,s1+1),ssLogL,method  = "Nelder-Mead")
      ...
      filenames = paste("./simulation/Table2(2)/",'p',p,'+',
                        'C',CensRateset,'+','Norm',ErrorNorm,"+",'CRCSIS','.csv',sep = '')
      ...
      for(i in 1:iteration){
        if(sum(SubS%in%S0)/length(SubS)<0.95){
          ...
          ssLogL <- function(sbeta) -sLogLiklihood(sdata,sbeta)
          results <- optim(rep(0,sp),ssLogL,"Nelder-Mead")
          Shat.u <- SubS
      ...
      filenames = paste("./simulation/Table2(2)/",'p',p,'+',
                        'C',CensRateset,'+','Norm',ErrorNorm,"+",'CRSJS','.csv',sep = '')
      ...
      filenames = paste("./simulation/Table2(2)/",'p',p,'+',
                        'C',CensRateset,'+','Norm',ErrorNorm,"+",'crCRS','.csv',sep = '')
      ...
      filenames = paste("./simulation/Table2(2)/",'p',p,'+',
                        'C',CensRateset,'+','Norm',ErrorNorm,"+",'crSIRS','.csv',sep = '')
      ...
      filenames = paste("./simulation/Table2(2)/",'p',p,'+',
                        'C',CensRateset,'+','Norm',ErrorNorm,"+",'SIS','.csv',sep = '')
    ...
    filenames = paste("./simulation/Table2(2)/",'p',p,'+',
                      'C',CensRateset,'+','Norm',ErrorNorm,"+",'Average','.csv',sep = '')
