...
## Screening2
##
#       function simulating cases in Table 2, part 1
#
...
Screening2 <- function(sim = 200,n = 200,p = 5000,Rhoset = 0.2,CensRateset = 20){
  set.seed(999)
sle_scr1 = matrix(NA,nrow = sim,ncol = 6) 
sle_scr2 = matrix(NA,nrow = sim,ncol = 6)
sle_scr3 = matrix(NA,nrow = sim,ncol = 6)
sle_scr4 = matrix(NA,nrow = sim,ncol = 6)
sle_scr5 = matrix(NA,nrow = sim,ncol = 6)
sle_scr6 = matrix(NA,nrow = sim,ncol = 6)
...
try({
  ...
  s = 5
  beta1 <- c(2,2,2,-0.8,-1.5,rep(0,(p-s)))
  beta2 <- c(-1.5,1.5,1.5,1.5,1.5,rep(0,(p-s)))
  S000 <- 1:s
  S111 <- (s+1):p
  ...
  CIF1I <- function(u,z){(-log(1-1/pp*(1-exp(exp(-z%*%beta1)*log(1-u*(1-(1-pp)^(exp(z%*%beta1))))))))/0.5}
  #CIF2I: function to generate F_2(t|Z)
  CIF2I <- function(u,z){(-log(1-u)*exp(-z%*%beta2))/0.8}
  TT <- c()
  u <- runif(n,0,1)
  ...
  if(CensRateset == 20)
    C = -log(u_new)/(0.05*exp((Z[,1]-Z[,2]))) else if(CensRateset == 40)
    C = -log(u_new)/(0.6*exp((Z[,1]-Z[,2]))) else if(CensRateset == 60) C = -log(u_new)/(2*exp((Z[,1]-Z[,2])))
  ...
  filenames = paste("./simulation/Table2/",'p',p,'+','rho',Rhoset*10,'+',
                  'C',CensRateset,'+','CRSIS','.csv',sep = '')
  ...
  sle_scr6[m,6] = ifelse(sum(sle_scr6[m,1:5]) == 5,1,0)
  filenames = paste("./simulation/Table2/",'p',p,'+','rho',Rhoset*10,'+',
                  'C',CensRateset,'+','SIS','.csv',sep = '')
