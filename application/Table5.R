...
SISY = X[which(delta == 1)]
SISX = Z[which(delta == 1),]  
crSIS = MFSIS(SISX,SISY,method = "SIS",nsis  =  q) 

write.csv(colnames(Z)[crSIS],"./application/TableS1/SIS-genes.csv")
...
write.csv(colnames(Z)[crCRS1],"./application/TableS1/crCRS-genes.csv")
...
SubS = order(crSIRS,decreasing  = TRUE)[1:q]

write.csv(colnames(Z)[SubS],"./application/TableS1/crSIRS-genes.csv")

## penalized estimation after crSIRS (Table5) ##
...
for(method in c("LASSO","MCP","SCAD")){
  ...
  write.csv(betala,paste("./application/Table5/crSIRS_",method,'estimates.csv',sep = ''))
}
...
write.csv(colnames(Z)[SubS],"~/application/TableS1/CRSJS-genes.csv")

## penalized estimation after CRSJS (Table 5)### 
...
betala <- fitla$beta[,5][which(fitla$beta[,5]!= 0)]
betala = betala[which(betala!= 0)]
write.csv(betala,"./application/Table5/CRSJS_LASSOestimates.csv")

fitsc <- crrp(ftime, fstatus, cov, penalty = "SCAD")
betasc <- fitsc$beta[,4][which(fitsc$beta[,4]!= 0)]
betasc = betasc[which(betasc!= 0)]
write.csv(betasc,"./application/Table5/CRSJS_SCADestimates.csv")

fitmc <- crrp(ftime, fstatus, cov, penalty = "MCP")
betamc <- fitmc$beta[,5][which(fitmc$beta[,5]!= 0)]
betamc = betamc[which(betamc!= 0)]
write.csv(betamc,"./application/Table5/CRSJS_MCPestimates.csv")
...
write.csv(colnames(Z)[SubS],"./application/TableS1/CRSIS-genes.csv")

## penalized estimation after CRSIS (Table5) ##
...
fitla <- crrp(ftime, fstatus, cov, penalty = "LASSO")
betala <- fitla$beta[,3][which(fitla$beta[,3]!= 0)]
betala = betala[which(betala!= 0)]
write.csv(betala,"./application/Table5/CRSIS_LASSOestimates.csv")

fitsc <- crrp(ftime, fstatus, cov, penalty = "SCAD")
betasc <- fitsc$beta[, which.min(fitsc$BIC)]
betasc = betasc[which(betasc!= 0)]
write.csv(betasc,"./application/Table5/CRSIS_SCADestimates.csv")

fitmc <- crrp(ftime, fstatus, cov, penalty = "MCP")
betamc <- fitmc$beta[, which.min(fitmc$BIC)]
betamc = betamc[which(betamc!= 0)]
write.csv(betamc,"./application/Table5/CRSIS_MCPestimates.csv")
...
write.csv(colnames(Z)[SubS],"./application/TableS1/CRCSIS-genes.csv")


#define new data subset of selected variable
ZFP = Z[,SubS]
...
## penalized estimates after CRCSIS (Table5) ##
fitla <- crrp(ftime, fstatus, cov, penalty = "LASSO")
betala <-  fitla$beta[,5][which(fitla$beta[,5]!= 0)]
betala = betala[which(betala!= 0)]
...
write.csv(betala,"./application/Table5/CRCSIS_LASSOestimates.csv")
write.csv(betasc,"./application/Table5/CRCSIS_SCADestimates.csv")
write.csv(betamc,"./application/Table5/CRCSIS_MCPestimates.csv")
